...
Limite dichiarato: le texture sono procedurali, non asset AGESCI reali — `docs/DESIGN.md` chiede realismo fotografico, raggiungibile solo con la pipeline PDF di Fase 3, che sostituirà le `map` senza toccare geometrie o interazione. Il frame rate reale su GPU fisica non è mai stato misurato in questo ambiente di sviluppo; la verifica su hardware reale è stata rimossa dal piano ([DEC-025](DECISIONS.md#dec-025--p10-t03-frame-rate-reali-su-device-mobile-rimosso-dal-piano)) — il budget quantitativo automatico (draw call/triangoli/texture, `tests/e2e/budget.ts`) resta la verifica di performance continua.
...
- attenzione a ombre, render loop e GPU mobile (frame rate reali su hardware fisico non misurati — verifica rimossa dal piano, [DEC-025](DECISIONS.md#dec-025--p10-t03-frame-rate-reali-su-device-mobile-rimosso-dal-piano)).
