...
- **Stato**: 6/8 flussi completati con E2E automatizzato reale (2026-08-25); 2 bloccati da un limite esterno (email), non da un problema del prodotto. Credenziali create (`qa-eg-a/b`, `qa-capo`, `qa-admin@ormaqa.test`, Reparto "test", env in `.env.local`). Durante questa verifica trovati e corretti 2 bug reali del prodotto (nome Maestro interno mai mostrato, sezione Maestro bloccata al primo utilizzo) più una vulnerabilità critica di sicurezza scoperta durante la preparazione degli account (auto-promozione a Capo/admin) — dettaglio in `.claude/CORRECTIONS.md`.
  1. **Completato**: `tests/e2e/percorsoPersonale.spec.ts` (nuovo) — login, avvia una Specialità dall'album, apre la carta, segna completata, nota (crea/modifica/elimina), associa Maestro interno via email. Trovato e corretto un bug reale: il nome del Maestro interno associato non veniva mai mostrato (embed PostgREST su `profiles` bloccato dalla RLS, conseguenza collaterale del fix P10-T01) — vedi `.claude/CORRECTIONS.md`. Associazione Maestro esterno e ricerca globale non ancora coperte da questo file (restano nel flusso 7/Maestri).
  4. **Completato**: `tests/e2e/repartoManagement.spec.ts` (nuovo) — Squadriglie (crea, rinomina, elimina) e Calendario (crea, modifica, elimina) come Capo, via gli oggetti guidone/calendario del tavolo reale.
  5. **Completato** nello stesso file di cui sopra (vedi punto 4) — il calendario di Reparto.
  6. **Completato**: `tests/e2e/archivio.spec.ts` (nuovo) — crea luogo, crea uscita collegata, carica una fotografia, elimina fotografia/uscita/luogo, tutto come Capo via l'oggetto baule.
  7. **Completato**: `tests/e2e/maestri.spec.ts` (nuovo) — opt-in Maestro dalla tessera, ricerca globale con filtro Zona, associazione a una Specialità in corso. Trovato e corretto un bug reale del prodotto (non del test): la sezione "Maestro di Specialità" restava bloccata su "Leggo la sezione…" per sempre al primissimo utilizzo (`!data` non distingue "non ancora caricato" da "caricato, nessuna riga esiste ancora") — mai notato perché ogni verifica precedente riusava account con quella sezione già compilata almeno una volta. Fix in `components/panel/surfaces/ImpostazioniSurface.tsx`, dettaglio in `.claude/CORRECTIONS.md`.
  2-3, 8. `tests/e2e/onboardingJourney.spec.ts` (nuovo, 4 test): **8. completato** (admin decide una richiesta reale, verificato anche che un non-admin/non-Capo venga reindirizzato fuori da `/admin`). **2-3 scritti ma bloccati da un limite esterno**: la registrazione via form reale invia una vera email di conferma tramite il servizio SMTP integrato di Supabase Auth (diverso da Resend, usato solo per il consenso genitoriale) — quota di poche email/ora sul piano free, esaurita dalle ripetute esecuzioni di test in questa sessione ("email rate limit exceeded"). Il codice di test è verificato fino al punto di invio (form compilato e inviato correttamente, nessun errore applicativo); il bypass della conferma via client admin (`email_confirm: true`) e i passi successivi (onboarding, approvazione, consenso genitoriale) restano da verificare quando la quota si resetta (tipicamente entro un'ora) o con un provider SMTP personalizzato configurato lato Supabase Auth per l'ambiente di test.
...
- **Stato**: punti 1/2/4/5 completati (2026-08-25), punto 3 completato con credenziali di test reali il 2026-08-25 (vedi sotto). **Trovato e corretto durante la preparazione degli account di test un problema critico non coperto da nessuno dei 5 criteri originali di questo task**: `ruolo`/`is_admin` su `profiles` non erano protetti dal trigger anti-self-update — qualunque utente autenticato poteva auto-promuoversi Capo o admin globale con una singola chiamata PostgREST dalla console del browser. Corretto e verificato empiricamente (`supabase/migrations/20260825080218_profiles_block_ruolo_is_admin_self_update.sql`, applicata al progetto reale); dettaglio completo in `.claude/CORRECTIONS.md`.
  ...
  3. **Eseguita con credenziali reali (2026-08-25)**: creati 4 account di test nel progetto Supabase reale (`qa-eg-a`/`qa-eg-b`/`qa-capo`/`qa-admin@ormaqa.test`, Reparto "test" `f1b84ef3-2bed-4919-b63d-96da77178343`, credenziali in `.env.local`, mai committato). Suite RLS completa: **23/23 test verdi** su tutti e 4 i file (`personalTables`, `reparto`, `maestri`, `archivio`). Un test (`user_tappa`, isolamento tra A e B) falliva alla prima esecuzione perché A e B condividono il Reparto: la policy `user_tappa_select_own` (P7-T01) rende visibili ai compagni di Reparto **tutte** le Tappe (non solo quelle completate, a differenza di Specialità/Competenza) — comportamento di design documentato nel commento della migrazione `reparto_funzionalita.sql`, non un bug. `tests/unit/rls/personalTables.rls.test.ts` aggiornato: il caso `user_tappa` è uscito dalla tabella di isolamento generico e ha ora un test dedicato che verifica la visibilità cross-Reparto in lettura e l'isolamento in scrittura. Trovato e corretto anche un bug dell'harness di test (non del prodotto): `reparto.rls.test.ts` non ripuliva mai la `richiesta_reparto` di prova (`richiesta_reparto` non ha policy di delete per il proprietario, per design — il cleanup con l'utente stesso falliva silenziosamente), causando un conflitto sull'indice unico a ogni riesecuzione; `afterAll` ora usa `createAdminClient()` per il solo cleanup — verificato eseguendo la suite due volte di fila. Dettaglio in `.claude/CORRECTIONS.md`.
