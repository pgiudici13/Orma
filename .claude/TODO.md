...
**Deploy**: la migrazione `20260823110000_maestri_ricerca_globale.sql` risulta applicata al progetto Supabase reale (`ouffyxrhxhzqcduvgpon`, verificato via MCP `list_migrations`/`list_tables`) — la nota precedente ("da applicare") era stale, il deploy era già avvenuto senza il passaggio di chiusura che le fasi 3/5/6/7 eseguivano sempre prima di dichiararsi "deploy incluso". `get_advisors` post-hoc ha infatti mostrato avvisi mai triagiati: `auth_rls_initplan` (`auth.uid()` non wrappato in `(select auth.uid())` su 7 policy di `maestro_profilo`/`maestro_specialita`) e `multiple_permissive_policies` (`maestro_profilo`/`maestro_specialita` con due policy SELECT permissive per `authenticated`, mai unite come fatto per `profiles`/`squadriglia` nelle fasi precedenti). Migrazione di fix scritta, `20260823130000_maestri_archivio_performance_fix.sql` (copre insieme Fase 8 e Fase 9) — **non ancora applicata al progetto reale**: l'applicazione di una migrazione al database di produzione richiede conferma esplicita del proprietario del progetto, non eseguita autonomamente in questa sessione.
...
**Deploy**: la migrazione `20260823120000_archivio_reparto.sql` risulta applicata al progetto Supabase reale (`ouffyxrhxhzqcduvgpon`, verificato via MCP `list_migrations`/`list_tables`) — stessa nota stale di Fase 8: il deploy era già avvenuto senza il passaggio di chiusura `get_advisors`. Avvisi mai triagiati: `unindexed_foreign_keys` (7 FK senza indice di copertura: `campo.created_by`, `uscita.created_by`, `campo_partecipante.profile_id`, `uscita_partecipante.profile_id`, `campo_squadriglia.squadriglia_id`, `uscita_squadriglia.squadriglia_id`, `documento_archivio.uploaded_by`) e `auth_rls_initplan` (`auth.uid()` non wrappato nelle 7 policy `*_select_reparto` di `luogo`/`uscita`/`campo`/i quattro join/`documento_archivio`). Migrazione di fix scritta, `20260823130000_maestri_archivio_performance_fix.sql` (copre insieme Fase 8 e Fase 9) — **non ancora applicata al progetto reale**, stesso motivo di Fase 8: richiede conferma esplicita prima di toccare il database di produzione.
